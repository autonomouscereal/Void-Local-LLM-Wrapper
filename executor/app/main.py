    ...
    while pending:
        ...
        for step_id in runnable:
            ...
            if not isinstance(res, dict):
                ...
                step_outputs_by_step_id[step_id] = {"tool_name": tool_name, "result": _canonical_tool_result(res)}
                ok = True
                ...
                logging.error(
                    f"[executor] step_id={step_id!r} tool={tool_name!r} FAILED code={code!r} msg={msg!r} detail={err_detail!r} env={json.dumps(env_fields, sort_keys=True)!r}"
                )
                ...
                if not error_dict.get("message"):
                    error_dict["message"] = msg or f"{tool_name} failed with status {status}"
                ...
                step_outputs_by_step_id[step_id] = {"tool_name": tool_name, "result": _canonical_tool_result(res or {})}
                ok = True
            end_payload = {
                "t": int(time.time()*1000),
                "event": "end",
