...
async def run_super_loop(task: str, repo_root: str, *, trace_id: str, step_tokens: int = 900) -> dict:
    ...
    arch_env = await committee_ai_text(
        messages=[{"role": "system", "content": "You are the Architect. Follow the instructions in the user message exactly."}, {"role": "user", "content": arch_in}],
        trace_id=trace_id,
        temperature=0.3,
    )
    arch_txt = ""
    ...
    impl_env = await committee_ai_text(
        messages=[{"role": "system", "content": "You are the Implementer. Follow the instructions in the user message exactly."}, {"role": "user", "content": impl_in}],
        trace_id=trace_id,
        temperature=0.3,
    )
    impl_txt = ""
    ...
    rev_env = await committee_ai_text(
        messages=[{"role": "system", "content": "You are the Reviewer. Follow the instructions in the user message exactly."}, {"role": "user", "content": rev_in}],
        trace_id=trace_id,
        temperature=0.3,
    )
    rev_txt = ""
