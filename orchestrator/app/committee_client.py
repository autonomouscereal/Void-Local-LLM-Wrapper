...
def call_ollama_sync(base_url: str, payload: Dict[str, Any], trace_id: str):
    ...
    parser = JSONParser()
    parsed_obj = parser.parse(raw_text, {})
    parsed = dict(parsed_obj) if isinstance(parsed_obj, dict) else {}
...
def committee_member_text_sync(member_id: str, messages: List[Dict[str, Any]], trace_id: str, temperature: float = 0.3):
    cfg = _participant_for(member_id=member_id)
    logger.info(f"committee_ai_text_sync: messages={messages}")
    base = (cfg.get("base") or "").rstrip("/") if isinstance(cfg, dict) else ""
    base = base or QWEN_BASE_URL
    model = (cfg.get("model") if isinstance(cfg, dict) else None) or QWEN_MODEL_ID

    payload = build_ollama_payload(messages=messages, model=model, num_ctx=DEFAULT_NUM_CTX)
    res = call_ollama_sync(base_url=base, payload=payload, trace_id=trace_id)
...
def committee_ai_text_sync(messages, trace_id: str, rounds: int = None, temperature: float = 0.3):
    member_id = "qwen"
    logger.info(f"committee_ai_text_sync: messages={messages}")
    msgs = messages if isinstance(messages, list) else []
    res = committee_member_text_sync(member_id, msgs, trace_id=trace_id)
    text = ""
