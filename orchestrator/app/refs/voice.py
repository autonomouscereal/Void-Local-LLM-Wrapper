...
from .registry import create_ref, refine_ref, list_refs
from .embeds import compute_voice_embedding, maybe_load
...
def _ensure_manifest_for_voice(voice_id: str, samples: List[str]) -> Tuple[Optional[Dict[str, Any]], List[str]]:
    """
    Ensure a refs manifest exists for the given voice_id and that all sample
...
def _match_existing_voice(sample_paths: List[str]) -> Optional[str]:
    ...
    """
    # Compute aggregate embedding for the incoming samples.
    from ..locks.builder import _blocking_voice_embedding  # local import to avoid cycles
