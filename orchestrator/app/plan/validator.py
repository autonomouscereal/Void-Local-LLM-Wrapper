'''
validator.py: Plan shape validator.
'''
...
def validate_plan(plan: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(plan, dict):
        ...
        return [{"step_id": None, "error": "invalid_plan_root"}]
    ...
    if not isinstance(steps, list):
        ...
        return [{"step_id": None, "error": "invalid_plan_type"}]
    for step in steps:
        if not isinstance(step, dict):
            errors.append({"step_id": None, "error": "invalid_step_type"})
            continue
        step_id = step.get("step_id")
        tool = step.get("tool")
        inputs = step.get("inputs") or step.get("args") or {}
        if not isinstance(step_id, str) or not step_id.strip():
            errors.append({"step_id": None, "error": "missing_step_id"})
            continue
        if tool not in REQUIRED_INPUTS:
            errors.append({"step_id": step_id, "error": f"unknown_tool:{tool}"})
            continue
        if not isinstance(inputs, dict):
            errors.append({"step_id": step_id, "error": "invalid_inputs_type"})
            continue
        for key in REQUIRED_INPUTS[tool]:
            if key not in inputs:
                errors.append({"step_id": step_id, "error": f"missing_input:{key}"})
    if errors:
