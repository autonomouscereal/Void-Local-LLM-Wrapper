...
def _resolve_bindings(graph: dict) -> dict:
    ks_id = _first_node_id_by_class(graph, "KSampler") or _first_node_id_by_class(graph, "KSamplerAdvanced")
    if not ks_id:
        return {}
    ...
    if not pos_id or not neg_id:
        ...
        return {}
    latent_id = _first_node_id_by_class(graph, "EmptyLatentImage") or _first_node_id_by_class(graph, "LatentImage")
    ckpt_id = _first_node_id_by_class(graph, "CheckpointLoaderSimple") or _first_node_id_by_class(graph, "CheckpointLoaderSimpleSDXL")
...
async def tool_run(req: Request):
    ...
    if name != "image.dispatch":
        ...
        if isinstance(err_obj, str):
            ...
            if isinstance(res, dict) and isinstance(res.get("traceback"), str):
                trace_val2 = res.get("traceback")
            details: dict = {}
            if trace_val2:
                details["traceback"] = trace_val2
    ...
    try:
        ...
        if inline is not None:
            ...
            wf_path = args.get("workflow_path") or os.getenv("COMFY_WORKFLOW_PATH") or "/workspace/services/image/workflows/stock_smoke.json"
                    ...
                    if not pos_id or not neg_id:
                        ...
                        if ckpt_id:
                            ...
                            if not pos_id:
                                ...
                                prompt_graph[new_pos_id] = {
                                    "class_type": "CLIPTextEncode",
                                    "inputs": {"clip": [str(ckpt_id), 1], "text": str(args.get("prompt") or "")},
                                }
                                ks_in["positive"] = [new_pos_id, 0]
                            if not neg_id:
                                new_neg_id = _next_id(prompt_graph)
                                prompt_graph[new_neg_id] = {
                                    "class_type": "CLIPTextEncode",
                                    "inputs": {"clip": [str(ckpt_id), 1], "text": str(args.get("negative") or "")},
                                }
            ...
            if _qa_analyze_image is not None and _qa_analyze_image_regions is not None:
                ...
                if isinstance(region_info, dict):
                    ...
                    if isinstance(agg, dict) and isinstance(img_qa, dict):
                        ...
                        trv = agg.get("text_readable_lock")
                        if isinstance(trv, (int, float)):
                            img_qa["text_readable_lock"] = float(trv)
