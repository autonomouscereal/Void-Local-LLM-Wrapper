...
# Add Tencent source code to path for SR pipeline
HUNYUAN_SRC = os.getenv("HYVIDEO_SRC_ROOT", "/opt/models/hunyuan15_src").strip()
if HUNYUAN_SRC and os.path.isdir(HUNYUAN_SRC) and HUNYUAN_SRC not in sys.path:
    sys.path.insert(0, HUNYUAN_SRC)
    logging.getLogger(__name__).info("added Tencent source to sys.path: %s", HUNYUAN_SRC)

# Import Tencent pipeline (will be None if repo not available)
TENCENT_PIPELINE_AVAILABLE = False
try:
    from hyvideo.pipelines.hunyuan_video_pipeline import HunyuanVideo_1_5_Pipeline
    from hyvideo.commons.infer_state import initialize_infer_state
    TENCENT_PIPELINE_AVAILABLE = True
    logging.getLogger(__name__).info("Tencent pipeline imports successful")
except ImportError as e:
    logging.getLogger(__name__).warning("Tencent pipeline not available (SR will use diffusers fallback): %s", e)
...
async def lifespan(app: FastAPI):
    ...
    global PIPE, PIPE_MODEL_ID, TENCENT_PIPE, SR_TRANSFORMER, SR_UPSAMPLER, SR_SCHEDULER

    # Try to load Tencent pipeline first (if available)
    if TENCENT_PIPELINE_AVAILABLE:
        try:
            logging.getLogger(__name__).info("loading Tencent pipeline (base + SR)")
            TENCENT_PIPE = _load_tencent_pipeline()
            if TENCENT_PIPE is not None:
                logging.getLogger(__name__).info("Tencent pipeline loaded successfully")
                PIPE = None  # Don't use diffusers pipeline if Tencent pipeline works
                PIPE_MODEL_ID = "tencent_pipeline"
            else:
                logging.getLogger(__name__).warning("Tencent pipeline load failed, falling back to diffusers")
        except Exception as e:
            logging.getLogger(__name__).error("Tencent pipeline load failed: %s", e, exc_info=True)
            TENCENT_PIPE = None
    
    # Fallback to diffusers pipeline if Tencent pipeline not available or failed
    if TENCENT_PIPE is None:
        PIPE = _load_base_pipe(model_id=DEFAULT_MODEL_ID)
        PIPE_MODEL_ID = DEFAULT_MODEL_ID if PIPE is not None else None

        # Load SR components (only needed if using diffusers pipeline)
        SR_TRANSFORMER, SR_UPSAMPLER, SR_SCHEDULER = _load_sr_components(SR_ROOT)

    logging.getLogger(__name__).info(
        "startup complete tencent_pipe=%s diffusers_pipe=%s sr_loaded=%s base_model=%s sr_root=%s",
        bool(TENCENT_PIPE),
        bool(PIPE),
        bool(SR_TRANSFORMER and SR_UPSAMPLER and SR_SCHEDULER) if TENCENT_PIPE is None else False,
        PIPE_MODEL_ID,
        SR_ROOT,
    )
...
DEFAULT_DEVICE = os.getenv("HYVIDEO_DEVICE", "auto")
...
# Tencent pipeline (alternative to diffusers pipeline)
TENCENT_PIPE: Optional[Any] = None  # HunyuanVideo_1_5_Pipeline if available

# SR components (loaded once, only used if Tencent pipeline not available)
...
def _live_max_memory(headroom_gib: float = 1.5) -> dict:
    """
    Compute max_memory dict based on CURRENT free VRAM per GPU.
    Returns dict like {0: "20.5GiB", 1: "18.2GiB", "cpu": "200GiB"}
    """
    mm = {}
    if not torch.cuda.is_available():
        mm["cpu"] = "200GiB"
    ...
    for i in range(torch.cuda.device_count()):
        try:
            free_b, total_b = torch.cuda.mem_get_info(i)  # free, total
            free_gib = free_b / (1024 ** 3)
            use_gib = max(free_gib - headroom_gib, 0.5)  # never give 0
            mm[i] = f"{use_gib:.1f}GiB"
        except Exception as e:
            logging.getLogger(__name__).warning("failed to query GPU %d memory: %s", i, e)
...
def _return_pipe_to_cpu(pipe: HunyuanVideo15Pipeline) -> None:
    """
    Helper to return all pipeline components to CPU.
    Call this once per stage (base or SR) after that stage completes.
    """
    ...
    try:
        ...
        logger.debug("Mode 3 cleanup: all components returned to CPU")
    except Exception as cleanup_err:
        logger.warning("Mode 3 cleanup failed: %s", cleanup_err)


def _jit_dispatch_transformer(transformer: torch.nn.Module, trace_id: str) -> torch.device:
    """
    JIT dispatch a transformer model across GPUs + CPU based on live free VRAM.
    Returns the execution device for generator creation.
    """
    logger = logging.getLogger(__name__)
    
    # Reset any previous maps/hooks
    if hasattr(transformer, "reset_device_map"):
        try:
            transformer.reset_device_map()
        except Exception:
            pass
    
    # Decide budgets based on CURRENT free VRAM
    max_memory = _live_max_memory(headroom_gib=2.0)
    logger.info("JIT dispatch transformer: max_memory=%s", max_memory)
    ...
    try:
        ...
        logger.info("JIT dispatch transformer: computed device_map (keys: %s)", list(device_map.keys())[:10])
    except Exception as e:
        logger.error("JIT dispatch transformer: infer_auto_device_map failed: %s", e, exc_info=True)
        # Fallback: put entire transformer on best GPU or CPU
        best = _find_best_gpu()
        target = f"cuda:{best}" if best is not None else "cpu"
        device_map = {"": target}  # whole transformer to one device
        logger.warning("JIT dispatch transformer: using fallback device_map (entire transformer to %s)", target)
    
    # Dispatch transformer across GPUs + CPU
    offload_dir = os.path.join(UPLOAD_DIR, "offload", trace_id)
    ...
    try:
        ...
        logger.info("JIT dispatch transformer: dispatched across devices")
    except Exception as e:
        logger.error("JIT dispatch transformer: dispatch_model failed: %s", e, exc_info=True)
        # Fallback: move to best GPU or CPU
        best = _find_best_gpu()
        target = f"cuda:{best}" if best is not None else "cpu"
        transformer.to(target)
        logger.warning("JIT dispatch transformer: moved to %s (fallback)", target)
    
    # Return execution device
    best = _find_best_gpu()
    target = f"cuda:{best}" if best is not None else "cpu"
    exec_dev = torch.device(target)
    logger.info("JIT dispatch transformer: execution device=%s", exec_dev)
    return exec_dev


def _jit_dispatch_for_request(pipe: HunyuanVideo15Pipeline, trace_id: str) -> torch.device:
    """
    Per-request JIT dispatch: reset any previous maps, compute fresh device_map
    based on live free VRAM, dispatch transformer across GPUs+CPU, place other
    components on best GPU or CPU.
    Returns the execution device for generator creation.
    """
    logger = logging.getLogger(__name__)
    
    # 1) Reset any previous maps/hooks so we can re-dispatch
    if hasattr(pipe, "reset_device_map"):
        try:
            pipe.reset_device_map()
        except Exception:
    ...
    # 2) Decide budgets based on CURRENT free VRAM
    max_memory = _live_max_memory(headroom_gib=2.0)
    logger.info("JIT dispatch: max_memory=%s", max_memory)
    
    # 3) Dispatch transformer using helper
    exec_dev = _jit_dispatch_transformer(pipe.transformer, trace_id)
    
    # 4) Keep text encoders + VAE on execution device (or CPU fallback)
    target = str(exec_dev)
    ...
    for comp_name in ["text_encoder", "text_encoder_2", "vae"]:
        ...
        if comp is not None:
            try:
                ...
                logger.warning("JIT dispatch: failed to move %s to %s, using CPU: %s", comp_name, target, e)
                try:
                    comp.to("cpu")
                except Exception:
                    pass
    
    logger.info("JIT dispatch: execution device=%s", exec_dev)
    return exec_dev
...
def _device():
    if not torch.cuda.is_available():
        return "cpu"
    if (DEFAULT_DEVICE or "").strip().lower() in ("auto", "best", ""):
        best = _find_best_gpu()
    ...
    return f"cuda:{best_gpu}" if best_gpu is not None else "cuda:0"
...
def _load_tencent_pipeline() -> Optional[Any]:
    """
    Load Tencent's HunyuanVideo_1_5_Pipeline using their own code.
    This handles both base generation and SR internally.
    """
    if not TENCENT_PIPELINE_AVAILABLE:
        return None
    ...
    try:
        # Initialize inference state (required by Tencent pipeline)
        initialize_infer_state()
        
        # Create pipeline using Tencent's factory method
        # This loads both base model and SR components from /opt/models/hunyuan
        pipe = HunyuanVideo_1_5_Pipeline.create_pipeline(
            pretrained_model_name_or_path=SR_ROOT,  # Path to weights (transformer, upsampler, scheduler, vae)
            transformer_version="720p_t2v",  # Base transformer version
            enable_offloading=True,  # Enable CPU offloading
            enable_group_offloading=True,  # Enable group-based offloading
            device=torch.device("cpu"),  # Start on CPU (Mode 3: JIT dispatch per request)
            transformer_dtype=_dtype_from_str(DEFAULT_TRANSFORMER_DTYPE),
        )
        
        logger.info("Tencent pipeline loaded dur_ms=%d", int((time.monotonic() - t0) * 1000))
        return pipe
    except Exception as e:
        logger.error("Tencent pipeline load failed: %s", e, exc_info=True)
        return None
...
def _load_base_pipe(model_id: str) -> Optional[HunyuanVideo15Pipeline]:
    ...
    t0 = time.monotonic()
    try:
        # Mode 3: Load pipeline to CPU only at startup (no static device_map)
        # This allows per-request JIT dispatch based on live free VRAM
        load_kwargs = {
            "torch_dtype": p_dtype,
            "local_files_only": True,
        }
        
        logging.getLogger(__name__).info("loading pipeline to CPU (Mode 3: JIT dispatch per request)")
        ...
        # Move entire pipeline to CPU - no static device placement
        pipe.to("cpu")
        ...
        # Mode 3: Enable VAE tiling only (no static device placement)
        if DEFAULT_VAE_TILING and hasattr(pipe, "vae") and hasattr(pipe.vae, "enable_tiling"):
            try:
                pipe.vae.enable_tiling()
                logging.getLogger(__name__).info("vae tiling enabled")
...
def _load_sr_components(sr_root: str) -> Tuple[Optional[torch.nn.Module], Optional[torch.nn.Module], Optional[FlowMatchEulerDiscreteScheduler]]:
    ...
    try:
        ...
        # SR upsampler - skip AutoModel loading if using Tencent pipeline
        # Tencent pipeline handles SR internally, so we don't need to load upsampler separately
        sr_upsampler = None
        if TENCENT_PIPELINE_AVAILABLE:
            logging.getLogger(__name__).info("SR upsampler: using Tencent pipeline (skip AutoModel loading)")
        else:
            # Fallback: try AutoModel if Tencent pipeline not available
...
def _decode_latents_to_np_video(pipe: HunyuanVideo15Pipeline, latents: torch.Tensor) -> np.ndarray:
    ...
    # Align latents to VAE device to avoid device mismatch
    vae_dev = next(vae.parameters()).device
    lat = latents
    if lat.device != vae_dev:
        lat = lat.to(device=vae_dev)
...
def _run_sr_720_to_1080(
    ...
    # Mode 3: JIT dispatch SR transformer based on live free VRAM
    sr_exec_dev = _jit_dispatch_transformer(SR_TRANSFORMER, trace_id)
    
    # Move SR upsampler to execution device (or CPU fallback)
    try:
        SR_UPSAMPLER.to(sr_exec_dev)
    except Exception as e:
        logger.warning("SR: failed to move upsampler to %s, using CPU: %s", sr_exec_dev, e)
        try:
            SR_UPSAMPLER.to("cpu")
            sr_exec_dev = torch.device("cpu")
        except Exception:
            pass
    
    # Move pipe components needed for SR to execution device
    for comp_name in ["vae", "text_encoder", "text_encoder_2"]:
        ...
        if comp is not None:
            try:
                ...
                logger.warning("SR: failed to move %s to %s, using CPU: %s", comp_name, sr_exec_dev, e)
                try:
                    comp.to("cpu")
                except Exception:
                    pass
    
    # Create generator on SR execution device
    gen_sr = torch.Generator(device=sr_exec_dev).manual_seed(seed ^ 0xA5A5A5A5) if sr_exec_dev.type != "cpu" else torch.Generator().manual_seed(seed ^ 0xA5A5A5A5)
    ...
    # Move lowres latents to SR execution device
    lr = lowres_latents
    if lr.device != sr_exec_dev:
        lr = lr.to(sr_exec_dev)
    
    # SR components are already dispatched and on correct devices
...
def _run_generate_dict(data: Dict[str, Any]):
    ...
    # Generator device will be set after JIT dispatch
    gen = None  # Will be created after JIT dispatch with correct execution device
    ...
    with _PIPE_LOCK:
        ...
        # --- Mode 3: JIT dispatch per request ---
        ...
        # JIT dispatch: compute fresh device_map based on live free VRAM and dispatch
        exec_dev = _jit_dispatch_for_request(pipe, trace_id)
        exec_dev_str = str(exec_dev)  # Store for logging/response
        
        # Create generator on the execution device (not DEFAULT_DEVICE)
        if exec_dev.type == "cpu":
            gen = torch.Generator().manual_seed(used_seed)
        else:
            gen = torch.Generator(device=exec_dev).manual_seed(used_seed)
        
        # Update kwargs with correct generator
        ...
        log.info("base done dur_ms=%d latents_shape=%s dtype=%s device=%s exec_dev=%s", 
                 dur_base_ms, tuple(lat0.shape), lat0.dtype, lat0.device, exec_dev_str)

        # Decode 720p for fallback (VAE must still be on correct device)
        t0d = time.monotonic()
        try:
        ...
        # Move latents to CPU before cleanup (for SR stage if needed)
        lat0_cpu = lat0.to("cpu") if lat0.device.type != "cpu" else lat0
        
        # Mode 3 cleanup: Return base pipeline components to CPU after decode
        # (SR stage will do its own dispatch if needed)
        _return_pipe_to_cpu(pipe)
        ...
        if SR_ENABLE and SR_TRANSFORMER and SR_UPSAMPLER and SR_SCHEDULER:
            try:
                # SR stage: use CPU latents, SR components will be dispatched if needed
                video_hr = _run_sr_720_to_1080(
                    ...
                    lowres_latents=lat0_cpu,
                    num_frames=int(kwargs.get("num_frames") or 121),
                    seed=used_seed,
                    exec_dev=exec_dev,  # Pass base exec_dev, SR will compute its own
                    trace_id=trace_id,
                )
        ...
        # Final cleanup: ensure everything is back to CPU after SR (if SR ran)
        # Also reset SR transformer dispatch
        _return_pipe_to_cpu(pipe)
        if SR_ENABLE and SR_TRANSFORMER:
            try:
                if hasattr(SR_TRANSFORMER, "reset_device_map"):
                    SR_TRANSFORMER.reset_device_map()
                SR_TRANSFORMER.to("cpu")
                if SR_UPSAMPLER:
                    SR_UPSAMPLER.to("cpu")
                torch.cuda.empty_cache()
            except Exception as cleanup_err:
                log.warning("SR cleanup failed: %s", cleanup_err)
